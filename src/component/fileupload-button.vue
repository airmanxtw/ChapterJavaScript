        ...
        <template v-slot:activator="{ on, attrs }">
            ...
            <v-dialog v-model="message" width="600" persistent>
                <v-card>
                    ...
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn color="green darken-1" text @click="iknow">
                            我知道了
                        </v-btn>
                    </v-card-actions>
                </v-card>
...
<script>
    export default {        
            ...
            fileupload: function () {
                ...
                Promise.all(uploadworks).then(function () {
                    if (THIS.successfiles.length > 0) 
                        THIS.successmessage = "已成功上傳:";
                    
                    if (THIS.errorfiles.length > 0) 
                        THIS.errormessage = "發生下列錯誤:";
        ...
        computed: {
            ...
            message: function () {
                return this.errormessage.length > 0 || this.successmessage.length > 0 ? true : false;
            },
            canupload: function () {
            ...
            sizelimit:function(){
                return this.$store.state.sizelimit;
            }
        },
        watch: {
