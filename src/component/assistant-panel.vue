    ...
    <div>
        <v-dialog v-model="assdialog" max-width="700">
            ...
            <v-card>
                    ...
                    <v-row>
                        <v-col cols="12" align-self="center">
                            <v-data-table :headers="headers"
                                          ...
                                          class="elevation-1" 
                                          @click:row="rowclick" 
                                          :footer-props="{disableItemsPerPage:true,itemsPerPageText:'每頁筆數'}"
                                          >                                                      
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="green darken-1" @click="assdialog=false" text>
                        關閉
                    </v-btn>
                </v-card-actions>
            </v-card>
            ...
            <v-card>
                ...
                <v-card-text>
                    <v-form ref="addform" lazy-validation>
                        <v-select :items="deptItems" :disabled="disabledauto"
                                  label="所屬單位"
                                  :value="deptno"
                                  item-text="text"
                                  item-value="key"
                        ...
                        <v-autocomplete ref="vauto" :disabled="disabledauto"
                                        v-model="model"
                                        :items="items"
                                        :loading="isLoading"
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="green darken-1" @click="adddialog=false" text>
                        取消
                    </v-btn>
...
<script>
    ...
    export default {
                    ...
                    this.axios.post(this.$store.state.absURL+'api/Assistant?info=' + encodeURIComponent(this.loginuser.info), {
                        ...
                        if (!THIS.disabledauto) {
                            ...
                            var obj = THIS.assItems.find(function (obj) {
                                return obj.user_id == response.data.user_id && obj.dept_no == response.data.dept_no;
                            });
        ...
        watch: {
            search: function (val) {                
                if (this.model == null && val != undefined && ((val.charCodeAt(0) >= 0 && val.charCodeAt(0) <= 127 && val.length == 8) || (val.charCodeAt(0) > 127 && val.length >= 2)) && (!this.isLoading)) {
                    this.isLoading = true;
                    var THIS = this;
                    var VAL = val;
