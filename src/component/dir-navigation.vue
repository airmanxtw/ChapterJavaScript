<template>
    <v-navigation-drawer app v-model="drawer" width="300">        
        <v-text-field class="ma-2" outlined
                        label="搜尋檔案"
                        append-icon="mdi-magnify" @keydown="searchkeydown" v-model="keyword"
    ...
    export default {
        ...
        watch: {
            active: function (newQuestion, oldQuestion) {                
                if (newQuestion.length > 0 && ((oldQuestion.length > 0 && newQuestion[0] != oldQuestion[0]) || (oldQuestion.length == 0))) {                    
        ...
        components:{
            'search-panel': function (resolve){require(['./search-panel.vue'], resolve)},          
        },
    }
</script>